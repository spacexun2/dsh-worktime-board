...
export function apply(ctx, config) {
    ...
    function eventFingerprint(sourceId, time, event) {
        if (event.type !== 'step/start' && event.type !== 'assistant/message' && event.type !== 'tool/call' && event.type !== 'tool/result' && event.type !== 'user/message' && event.type !== 'step/end')
            return undefined;
    ...
    function pruneSeenEvents(sourceId) {
        const cursor = backfillCursor.get(sourceId) ?? 0;
        for (const [key, event] of seenEvents) {
            if (event.sourceId === sourceId && event.time < cursor)
    ...
    const foldLive = (srcId, t, event) => {
        ...
        if (fingerprint !== undefined && seenEvents.has(fingerprint))
            return;
        ...
        if (fingerprint !== undefined)
            seenEvents.set(fingerprint, { sourceId: srcId, time: t });
        markDirty();
    };
    ...
    function flushPendingLive() {
        ...
        for (const p of q) {
            ...
            if (p.t < cursor || p.t === cursor && (fingerprint === undefined || seenEvents.has(fingerprint)))
                continue;
    ...
    async function foldLogFile(path, minTime) {
        ...
        while (offset < buf.length) {
            ...
            for (const line of text.split('\n')) {
                ...
                if (fingerprint !== undefined)
                    seenEvents.set(fingerprint, { sourceId, time: t });
            }
    ...
    function calendarKeysForRange(range) {
        const today = dayKeyOf(new Date());
        if (range === 'day')
            return [today];
    ...
    function summarizeThreadForRange(threadId, range) {
        let activeMinutes = 0;
        let xianActive = 0;
        let yangActive = 0;
        let activeDays = 0;
    ...
    const disposeTool = ctx.tools.register({
        ...
        execute: async (args, exec) => {
            try {
                ...
                if (args?.ranch !== true && callerId === undefined)
                    return { text: '无法确定调用线程；请改用 ranch:true 查询全部线程' };
