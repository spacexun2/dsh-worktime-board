...
import { SLOTS_PER_DAY, createRecord, dayKeyOf, deserializeRecord, orSlots, serializeRecord, slotOf, summarizeRanch, summarizeThread, computeRealm, realmAvgForDays, restTitlePrefix, aggregateRange, calendarDays, calendarRange, setRealmCoeffs, getRealmCoeffs, applyBreakthrough, tomatoSegs, tomatoGrowth, realmOf, } from './core.js';
...
export function apply(ctx, config) {
     ...
     *  v11：重建保留生涯结算状态（careerMaxTier/failPenalty/failedCount）——防重建后单次结算再次一次性灌档；
     *  单次结算最多突破 1 档（一天一结算）；score.realm 按最终 value 映射；周/月成长系数用日均段数。 */
    ...
    function load() {
        try {
            ...
            if (parsed.meta?.schemaVersion !== SCHEMA_VERSION) {
                ...
                // v11：重建保留生涯结算状态——防重建后单次结算再次一次性灌档（档位/罚金/失败数为生涯属性，不随数据重建重置）
    ...
    function buildRange(range, date) {
        ...
        // 入定段数：day = 今日；week/month = 窗口内活跃日**日均**段数（v11 修正：不再 Σ 30 天段数，
        // 防成长系数 ×(1+Σ×0.1) 叠加把周/月境界虚高到宇宙洪荒）
        const segsDay = tomatoSegs(todayRanch.heatRealm);
        const segsWeekSum = all.slice(-7).reduce((s, d) => s + tomatoSegs(d.ranch.heatRealm), 0);
        const segsMonthSum = all.slice(-30).reduce((s, d) => s + tomatoSegs(d.ranch.heatRealm), 0);
        const activeDaysInWeek = Math.max(1, all.slice(-7).filter((d) => d.ranch.activeMinutes > 0).length);
        const activeDaysInMonth = Math.max(1, all.slice(-30).filter((d) => d.ranch.activeMinutes > 0).length);
        const segsWeek = segsWeekSum / activeDaysInWeek;
        const segsMonth = segsMonthSum / activeDaysInMonth;
        // 突破判定用三口径最高基础值，加奖励/罚金后可能连锁突破/失败回退
        // 存量数据豁免（用户定稿）：历史回填未完成（backfillComplete=false）时 rng 恒 1 → 不触发晋升失败；只对实时新晋升掷骰
        // v11 一天一结算：maxSteps=1 —— 单次结算最多突破 1 档（连锁突破拆到后续结算日逐日推进，突破次数随时间真实增长）
        const highestBase = Math.max(baseDay.value, baseWeek.value, baseMonth.value);
        const br = applyBreakthrough(highestBase, careerMaxTier, failPenalty, backfillComplete ? Math.random : () => 1, 1);
        if (br.failed) {
        ...
        const score = {
            ...scoreRange,
            realm: realmOf(scoreRange.value), // v11：境界按最终 value（含突破奖励/入定成长）映射，与总分显示一致
            realmByPeriod: {
