...
`;
		...
		/** 境界变比阈值表（下限含，50 起点，用户定稿）：<50 炼气 / 50 筑基 / 250 金丹 / 1250 元婴 / 6250 化神 / 31250 炼虚 / 10万 合体 / 20万 大乘 / 35万 渡劫 / 50万 真仙 / 70万 金仙 / 100万+ 宇宙洪荒。 */
		...
		*  cap：可选字号上限（分数卡一行放不下时压缩，如 40px 宇宙洪荒 + 7 位数字会换行）。 */
		...
		var WorktimePanel = class {
			...
			constructor(ctx, host) {
				...
				try {
					const period = localStorage.getItem(REALM_PERIOD_KEY);
			...
			*  串行锁：常规轮询忙时跳过；force（tab 切换）不碰锁，靠响应自带 range 校验过期（无序号无缓存）。 */
			async tick(force = false) {
				...
				try {
					...
					if (this.disposed) return;
					if (this.open && body.range !== this.tab) return;
      ...
      <button type="button" class="wtb-badge" data-busy="${busy}" data-phase="${this.badgePhase()}" aria-label="牛马修仙看板">
        ...
        <span class="wtb-badgeSwitch" data-badge-switch title="切换显示：时长 / 修仙值" aria-label="切换显示内容">⇄</span>
				...
				this.host.querySelector(".wtb-badge")?.addEventListener("click", () => {
					this.open = true;
					this.reschedule();
					if (this.data === null || this.data.range !== this.tab) this.tick(true);
					this.render();
				});
        ...
        <div class="wtb-verse">
          <div class="wtb-compPair" title="思考 = 模型生成耗时 · 执行 = 工具运行耗时">
            <div class="wtb-compTop">
              ...
              <span class="wtb-compName wtb-compName-right"><span class="wtb-compDot" style="background:#ff7043"></span><span class="wtb-compLabel">⚙️ 执行</span><b>${o.toolMs > 0 ? fmtDur(o.toolMs / 6e4) : "0"}</b><span class="wtb-compPct" style="color:#ff7043">${Math.round(o.toolRatio * 100)}%</span></span>
            </div>
          ...
          <div class="wtb-compPair" title="普通时间 vs 修仙时间（22:30–06:30 活动占比）">
			...
			scoreBlockHtml(o, s) {
				...
				const realm = period?.realm !== void 0 && period.realm !== "" ? period.realm : s.realm !== "" ? s.realm : realmOfLocal(value);
      ...
      <div class="wtb-scoreCard">
        ...
        ${this.showRealmDetail ? this.realmDetailHtml(value, tier, rb !== void 0, coeffsOf(s)) : ""}
        <div class="wtb-dims">
        ...
        <div class="wtb-quip">${quipLine(tier, minutesOfDay, this.realmPeriod)}</div>
			...
			realmDetailHtml(value, tier, hasPeriods, coeffs) {
      ...
      ${[
					...
					["week", "近7天"],
					["month", "近30天"]
				].map(([k, label]) => `<button type="button" class="wtb-realmPeriodBtn" data-realm-period="${k}" data-active="${this.realmPeriod === k}">${label}</button>`).join("")}
    </span></div>` : "";
				const periodHint = hasPeriods && this.realmPeriod !== "day" ? `<span class="wtb-realmPeriodHint">（${this.realmPeriod === "week" ? "近 7 天" : "近 30 天"} · 日均 ${fmtGate(Math.round(value))}）</span>` : "";
				return `
          ...
          <div class="wtb-yearStat"><span class="wtb-yearStatLabel">🌙 修仙时段</span><b>${Math.round(rec.xianPct * 100)}%</b></div>
				...
				root.querySelectorAll(".wtb-realmPeriodBtn").forEach((el) => {
					el.addEventListener("click", (e) => {
						e.stopPropagation();
						this.realmPeriod = el.getAttribute("data-realm-period");
						try {
							localStorage.setItem(REALM_PERIOD_KEY, this.realmPeriod);
						} catch {}
						this.render();
					});
				});
				...
				if (head !== null) head.addEventListener("pointerdown", (e) => {
					if (e.target.closest(".wtb-tab, .wtb-close") !== null) return;
