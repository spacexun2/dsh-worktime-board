...
export function apply(ctx: AppContext, config: Config): void {
  ...
  function buildRange(range: 'day' | 'week' | 'month', date?: string): unknown {
    ...
    let overview: {
      activeMinutes: number; peakParallel: number; calls: number; inputTokens: number;
      outputTokens: number; llmMs: number; toolMs: number; llmRatio: number; toolRatio: number;
      xianPct: number; activeDays: number; turns: number; steps: number; userInputs: number;
      humanInputs: number; avgActiveMinutes: number;
    }
    if (range === 'day') {
      overview = {
      ...
      for (const d of days) {
        const r = d.ranch
        if (r.activeMinutes <= 0) continue
        activeDays++
        activeMinutes += r.activeMinutes
        if (r.peakParallel > peakParallel) peakParallel = r.peakParallel
        calls += r.calls
    ...
    let score: Record<string, unknown> = dayScore
    if (range !== 'day') {
      ...
      let activeDays = 0
      for (const d of days) {
        if (d.ranch.activeMinutes <= 0) continue
        activeDays++
    ...
    const finalRealmOf = (day: string, ranch: ReturnType<typeof summarizeRanch> | undefined): number => {
